...
async def create_profile(profile: profiles.CreateProfile):
    ...
    profile_obj = await profiles.Profile.create(
...
@profile_router.put(
    '/profile/{profile_id}',
    response_model=profiles.Profile_Pydantic
)
async def update_profile(profile_id: str, profile: profiles.UpdateProfile):
    await profiles.Profile.filter(id=profile_id).update(
        **profile.dict(exclude_unset=True))
    return await profiles.Profile_Pydantic.from_queryset_single(
        profiles.Profile.get(id=profile_id))


@profile_router.put(
    '/profile/add-city/{profile_id}',
    response_model=profiles.Profile_Pydantic
)
async def add_profile_city(
    profile_id: str,
    cities_data: profiles.ProfileAddCity
):
...
async def get_my_profile(
    user: users.User_Pydantic = Depends(Auth.get_current_user)
):
